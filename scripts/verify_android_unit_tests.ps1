...
if (-not (Test-Path -LiteralPath $gradle)) {
    ...
    if ($null -eq $gradleCommand) {
        if (-not (Test-Path -LiteralPath $bootstrapGradleScript -PathType Leaf)) {
            throw "Gradle not found. Install Gradle 9.5.1 or put gradle on PATH."
        }
        & powershell -NoProfile -ExecutionPolicy Bypass -File $bootstrapGradleScript | Out-Host
        if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $defaultGradle -PathType Leaf)) {
            throw "Gradle bootstrap failed. Install Gradle 9.5.1 or put gradle on PATH."
        }
        $gradle = $defaultGradle
    } else {
