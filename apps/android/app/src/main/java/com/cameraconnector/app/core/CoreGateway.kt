...
interface CoreGateway {
    ...
    suspend fun loadGlobalPromptPacks(): List<PromptPackUi>
    ...
    suspend fun forkGlobalPromptPack(
        sourcePackId: String,
        name: String,
        distributionFolder: String,
    ): PromptPackUi
    ...
    suspend fun loadPromptPacks(projectId: String): List<PromptPackUi>
    suspend fun forkPromptPack(
        projectId: String,
        sourcePackId: String,
        name: String,
    ...
    suspend fun createManualBurstGroup(projectId: String, memberGroupIds: List<String>)
}
...
class PreviewCoreGateway : CoreGateway {
    ...
    override suspend fun deleteProject(projectId: String) {
        val nextProjects = projects.value.projects.filterNot { it.id == projectId }
        val nextActiveProjectId = projects.value.activeProjectId
            .takeUnless { it == projectId }
            ?: nextProjects.firstOrNull { it.status.equals("Active", ignoreCase = true) }?.id
        projects.value = ProjectState(
            projects = nextProjects,
            activeProjectId = nextActiveProjectId,
        )
    ...
    override suspend fun createGlobalPromptPack(
        ...
        PromptPackUi(
            promptPackId = "global-custom-${name.ifBlank { "prompt" }}",
    ...
    override suspend fun forkGlobalPromptPack(
        ...
        previewPromptPacks("")
            .firstOrNull { it.promptPackId == sourcePackId }
            ?.let { source ->
                source.copy(
                    promptPackId = "global-$sourcePackId",
                    distributionFolder = distributionFolder.ifBlank { "user" },
                    scope = "global",
                    projectId = null,
    ...
    override suspend fun saveGlobalPromptPack(
        ...
        previewPromptPacks("")
            .firstOrNull { it.promptPackId == promptPackId }
            ?.copy(
                name = name,
                styleTags = styleTags,
    ...
    override suspend fun forkPromptPack(
        ...
        previewPromptPacks(projectId)
            .firstOrNull { it.promptPackId == sourcePackId }
            ?.copy(
                promptPackId = "project-$sourcePackId",
                distributionFolder = distributionFolder.ifBlank { "user" },
                scope = "project",
                projectId = projectId,
    ...
    override suspend fun savePromptPack(
        ...
        previewPromptPacks(projectId)
            ...
            ?: forkPromptPack(projectId, promptPackId, name)
    ...
    override suspend fun createManualBurstGroup(projectId: String, memberGroupIds: List<String>) = Unit
}
...
private fun previewPromptPacks(projectId: String): List<PromptPackUi> =
    listOf(
        PromptPackUi(
            ...
            projectId = null,
            name = "通用评价",
            styleTags = listOf("通用", "均衡", "基础"),
            sceneProfile = "general",
            activeVersionId = "general-default-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "通用摄影评价规则，兼顾主体价值、光线、构图、技术底线、情绪记忆点、淘汰标准和连拍优选。",
            sharedPreference = "通用摄影评价规则，兼顾主体价值、光线、构图、技术底线、情绪记忆点、淘汰标准和连拍优选。",
        ),
        PromptPackUi(
            promptPackId = "documentary-integrity",
            scope = "global",
            projectId = null,
            name = "纪实真实",
            styleTags = listOf("纪实", "新闻", "叙事"),
            sceneProfile = "general",
            activeVersionId = "documentary-integrity-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "纪实新闻规则，强调事实伦理、信息密度、决定性瞬间、系列叙事和避免误导性选择。",
            sharedPreference = "纪实新闻规则，强调事实伦理、信息密度、决定性瞬间、系列叙事和避免误导性选择。",
        ),
        PromptPackUi(
            promptPackId = "portrait-editorial",
            scope = "global",
            projectId = null,
            name = "人像编辑",
            styleTags = listOf("人像", "编辑", "情绪"),
            sceneProfile = "portrait",
            activeVersionId = "portrait-editorial-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "编辑型人像规则，强调身份表达、眼神姿态、光线风格、环境造型和封面/专题价值。",
            sharedPreference = "编辑型人像规则，强调身份表达、眼神姿态、光线风格、环境造型和封面/专题价值。",
        ),
        PromptPackUi(
            promptPackId = "portrait-lifestyle",
            scope = "global",
            projectId = null,
            name = "写真创作",
            styleTags = listOf("写真", "情绪", "风格", "生活方式"),
            sceneProfile = "portrait",
            activeVersionId = "portrait-lifestyle-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "写真创作规则，强调故事感、情绪、日系空气感、运动动态、广角冲击和剪影逆光。",
            sharedPreference = "写真创作规则，强调故事感、情绪、日系空气感、运动动态、广角冲击和剪影逆光。",
        ),
        PromptPackUi(
            promptPackId = "portrait-conservative",
            scope = "global",
            projectId = null,
            name = "人像稳健",
            styleTags = listOf("人像", "稳健", "交付"),
            sceneProfile = "portrait",
            activeVersionId = "portrait-conservative-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "稳健人像交付规则，优先人物状态、面部质量、肤色、尊严感和客户可用性。",
            sharedPreference = "稳健人像交付规则，优先人物状态、面部质量、肤色、尊严感和客户可用性。",
        ),
        PromptPackUi(
            promptPackId = "landscape-fine-art",
            scope = "global",
            projectId = null,
            name = "风光艺术",
            styleTags = listOf("风光", "艺术", "氛围"),
            sceneProfile = "landscape",
            activeVersionId = "landscape-fine-art-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "艺术风光规则，强调气氛、构成关系、光影层次、色彩克制和作者性。",
            sharedPreference = "艺术风光规则，强调气氛、构成关系、光影层次、色彩克制和作者性。",
        ),
        PromptPackUi(
            promptPackId = "landscape-technical",
            scope = "global",
            projectId = null,
            name = "风光技术",
            styleTags = listOf("风光", "技术", "清晰"),
            sceneProfile = "landscape",
            activeVersionId = "landscape-technical-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "技术风光规则，强调光线质量、空间层次、动态范围、地平线、色彩调性和输出完整度。",
            sharedPreference = "技术风光规则，强调光线质量、空间层次、动态范围、地平线、色彩调性和输出完整度。",
        ),
        PromptPackUi(
            promptPackId = "wildlife-ethics",
            scope = "global",
            projectId = null,
            name = "野生自然",
            styleTags = listOf("野生", "自然", "伦理"),
            sceneProfile = "landscape",
            activeVersionId = "wildlife-ethics-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "野生自然规则，强调真实行为、生境叙事、眼部主体质量、稀缺性和拍摄伦理。",
            sharedPreference = "野生自然规则，强调真实行为、生境叙事、眼部主体质量、稀缺性和拍摄伦理。",
        ),
        PromptPackUi(
            promptPackId = "action-sports-moment",
            scope = "global",
            projectId = null,
            name = "运动瞬间",
            styleTags = listOf("运动", "动作", "速度"),
            sceneProfile = "action",
            activeVersionId = "action-sports-moment-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "运动动作规则，强调峰值动作、身体线条、速度表达、背景分离和胜负情绪。",
            sharedPreference = "运动动作规则，强调峰值动作、身体线条、速度表达、背景分离和胜负情绪。",
        ),
        PromptPackUi(
            promptPackId = "architecture-design",
            scope = "global",
            projectId = null,
            name = "建筑空间",
            styleTags = listOf("建筑", "空间", "秩序"),
            sceneProfile = "custom",
            activeVersionId = "architecture-design-v1",
            builtIn = true,
            enabled = true,
            activePromptText = "建筑空间规则，强调线条比例、透视控制、光影材质、尺度关系和项目叙事。",
            sharedPreference = "建筑空间规则，强调线条比例、透视控制、光影材质、尺度关系和项目叙事。",
        ),
    )
