...
| RX-014A | Dashboard read-model | P0 | Core exposes one dashboard query for UI shells with safe account summaries, receiver status, transfer health counts, connected devices, filtered asset summary, and paged asset groups; CLI can emit the same model as JSON for app shells and automation |
