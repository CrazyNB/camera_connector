...
- Publish workers must claim a pending `publish_queue` item before writing to final storage; claiming moves `staged` or `failed` items to `publishing` so retry workers do not publish the same staged bytes twice.
