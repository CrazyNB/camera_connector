...
Account passwords are accepted as setup input only. The SQLite account table stores `password_hash` generated by the core account model, and `account list` only reports whether a password is configured.
