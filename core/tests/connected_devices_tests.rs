use camera_connector_core::{
    ...
    record_device_connected, record_device_disconnected, FtpPushServer, PushProtocol,
    PushReceiverConfig, SqliteStore,
};
...
#[test]
fn connected_device_state_is_stored_in_sqlite() {
    let temp_dir = tempfile::tempdir().expect("temp dir should be created");
    ...
    assert!(!connected_devices_path(temp_dir.path()).exists());
    let store = SqliteStore::open_state_dir(temp_dir.path()).expect("store should open");
    let devices = store
        .connected_devices()
        .expect("devices should read from sqlite");
