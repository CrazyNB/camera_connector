...
fn burst_selection_recommendation_uses_configured_model_provider_decision() {
    ...
    assert_eq!(recommendation.selected_asset_group_ids, vec![second_id.clone()]);
    assert_eq!(recommendation.candidate_asset_group_ids, vec![first_id.clone()]);
    assert_eq!(
...
fn burst_selection_recommendation_can_use_visual_candidates_without_prior_model_evaluations() {
    ...
    service
        .record_project_transfer(
            &project.project_id,
            completed_transfer("ftp:burst-visual-1", "DCIM/100/BURST_VISUAL_0001.JPG", 1_000),
        )
        .expect("first transfer should record");
    service
        .record_project_transfer(
            &project.project_id,
            completed_transfer("ftp:burst-visual-2", "DCIM/100/BURST_VISUAL_0002.JPG", 1_100),
        )
        .expect("second transfer should record");
    service
        .drain_analysis_jobs_with_provider_configured(10, false)
    ...
    assert_eq!(recommendation.source, SelectionSource::Llm);
    assert_eq!(recommendation.status, SelectionRecommendationStatus::Pending);
    assert_eq!(recommendation.selected_asset_group_ids, Vec::<String>::new());
    assert_eq!(
