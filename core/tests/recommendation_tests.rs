use camera_connector_core::{
    ...
    ModelProviderKind, ModelProviderSettings, ModelSendMode, PreviewSample, QualityAnalysisStatus,
    QualityScore, SelectionRecommendationScope, SelectionRecommendationStatus, SelectionSource,
    SignalScore, StoredObjectLocation, StrategyProfile, TechnicalAssessment,
    TechnicalAssessmentStatus, TechnicalGateStatus, TransferRecord, TransferStatus,
};
...
fn project_model_recommendation_selects_good_singletons_and_strong_burst_winners() {
    ...
    let project_recommendation = recommend_project_selects(
        "project-1",
        &[
...
#[test]
fn service_score_preview_persists_technical_and_model_evaluation_records() {
    let temp_dir = tempfile::tempdir().expect("temp dir should create");
    let service = CameraConnectorService::new(Some(temp_dir.path().join("config.json")));
    let project = service
